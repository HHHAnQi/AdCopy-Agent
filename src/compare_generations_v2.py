...
def main():
    ...
    parser.add_argument("--dpo_v2_file", default="outputs/eval_reports/dpo_v2_generations.jsonl")
    parser.add_argument("--output_file", default="docs/comparison_samples_v2.md")
    ...
    dpo_v2_rows = to_map(read_jsonl(Path(args.dpo_v2_file)))

    common_ids = [
        x for x in base_rows.keys()
        if x in sft_rows and x in dpo_rows and x in dpo_v2_rows
    ][: args.num_samples]
    ...
    lines = []
    lines.append("# Base / SFT / DPO v1 / DPO v2 Generation Comparison\n")
    lines.append(
        "This file compares generation outputs from the base model, SFT model, "
        "DPO v1 model, and DPO v2-small model on the same final-test samples.\n"
    )
    ...
    for idx, sid in enumerate(common_ids, start=1):
        ...
        lines.append("### DPO v1 Output\n")
        lines.append(f"{shorten(d1.get('prediction', ''))}\n")
        lines.append("### DPO v2-small Output\n")
        lines.append(f"{shorten(d2.get('prediction', ''))}\n")
        lines.append("---\n")
